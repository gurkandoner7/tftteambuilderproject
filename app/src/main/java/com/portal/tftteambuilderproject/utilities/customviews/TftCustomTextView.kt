...
import com.portal.tftteambuilder.databinding.TftCustomTextViewBinding
import com.portal.tftteambuilderproject.utilities.extensions.setColoredSpan
import com.portal.tftteambuilderproject.utilities.extensions.withClickableSpan

class TftCustomTextView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : LinearLayout(context, attrs, defStyleAttr) {
    private var _binding: TftCustomTextViewBinding? = null
    private val binding: TftCustomTextViewBinding
        get() = _binding!!

    private val attributes =
        context.obtainStyledAttributes(attrs, R.styleable.TftCustomTextView)
    private val _textSize = attributes.getDimension(
        R.styleable.TftCustomTextView_android_textSize,
        12F
    )
    private val textColor = attributes.getColor(
        R.styleable.TftCustomTextView_android_textColor,
        ContextCompat.getColor(context, R.color.white)
    )
    private val _maxLines =
        attributes.getInt(R.styleable.TftCustomTextView_android_maxLines, -1)
    private val _ellipsize =
        attributes.getInt(R.styleable.TftCustomTextView_android_ellipsize, 0)

    init {
        _binding = TftCustomTextViewBinding.inflate(LayoutInflater.from(context), this, true)
        initAttributes()
    }

    private fun initAttributes() {
        when (attributes.getInteger(R.styleable.TftCustomTextView_textType, 0)) {
        ...
        binding.textView.text = attributes.getString(R.styleable.TftCustomTextView_android_text)
        ...
        when (attributes.getInteger(R.styleable.TftCustomTextView_textAlignment, 0)) {
